...
using PLUG.System.Common.Extensions;
    ...
    RabbitMQTelemetry rabbitMQTelemetry) : IEventBus, IDisposable, IHostedService
{
    ...
    public Task PublishAsync(IntegrationEvent @event)
    {
        var policy = Policy.Handle<BrokerUnreachableException>()
            .Or<SocketException>()
            .WaitAndRetryAsync(this._retryCount, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)));
    ...
    private async Task ProcessEvent(string eventName, string message)
    {
        ...
        this._subscriptionInfo.HandlerTypes.TryGetValue(eventType, out var handlerTypes);

        handlerTypes ??= new HashSet<Type>();
